...
function initializeLeafletMap(frmInstance) {
	...
	function loadMap(region_code, currentLevel, parentLevel, renderLevel) {
		...
		const url = `polmark_dashboard.api.geojson.get_geojson_data_by_region?region=Kota Medan&region_level=${mapRenderLevel}&region_code=${region_code}`;
	...
	function createDataTooltip() {
		let tableHeader = `<div id="databox-tooltip" style="position:absolute; bottom:25px; right:18px; padding:10px; background-color:white; border:1px solid #ccc; display:none; z-index:1000;">
		<table id="tooltip-table" style="border-collapse: collapse; width: 100%;">
			<tbody>
				...
				<tr>
					<th style="text-align: left; padding: 5px; border-bottom: 1px solid #ccc;">DPTHP 2</th>
	...
	function renderTable(level, data) {
		...
		table += voterHeader + "</tr></thead><tbody>";
		...
		data.forEach((item) => {
			...
			<td>${item.num_voter_per_family_dpthp2}</td>
	...
	function fetchTableData(level, region) {
		const url = `polmark_dashboard.api.tabular.get_tabular_data?region=Kota Medan&region_level=${mapRenderLevel}&region_code=${region}`;
