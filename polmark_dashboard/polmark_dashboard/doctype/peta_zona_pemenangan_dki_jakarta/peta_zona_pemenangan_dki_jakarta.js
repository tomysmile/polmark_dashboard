...
frappe.ui.form.on("Peta Zona Pemenangan DKI Jakarta", {
	refresh(frm) {
...
function initializeLeafletMap(frmInstance) {
	...
	function loadMap(region_code, currentLevel, parentLevel, renderLevel) {
		...
		const url = `polmark_dashboard.api.geojson.get_geojson_data_by_region?region=DKI Jakarta&region_level=${mapRenderLevel}&region_code=${region_code}`;
	...
	function fetchTableData(level, region) {
		const url = `polmark_dashboard.api.tabular.get_tabular_data?region=DKI Jakarta&region_level=${mapRenderLevel}&region_code=${region}`;
