...
frappe.ui.form.on("Peta Zona Pemenangan Kota Bandung", {
	refresh(frm) {
...
function initializeLeafletMap(frmInstance) {
	...
	function renderMap(level, geoJsondata) {
		if (!isNavigatingBack) {
			...
			if (mapLevelStack.length === 0 || mapLevelStack[mapLevelStack.length - 1] !== level) {
	...
	function loadMap(region_code, currentLevel, parentLevel, renderLevel) {
		...
		const url = `polmark_dashboard.api.geojson.get_geojson_data_by_region?region=Kota Bandung&region_level=${mapRenderLevel}&region_code=${region_code}`;
	...
	function showHideDataBoxTooltip(isShow = true) {
		document.getElementById("databox-tooltip").style.display = isShow ? "block" : "none";
	}
	...
	function createDataTooltip() {
		...
		<table id="tooltip-table" style="border-collapse: collapse; width: 100%;">
			<tbody>
				...
				<tr>
					<th style="text-align: left; padding: 5px; border-bottom: 1px solid #ccc;">DPTHP2</th>
					<td id="jml-dpthp" style="padding: 5px; border-bottom: 1px solid #ccc;"></td>
				</tr>
				<tr>
					<th style="text-align: left; padding: 5px; border-bottom: 1px solid #ccc;">DPTHP2 Perempuan</th>
					<td id="jml-dpthp-perempuan" style="padding: 5px; border-bottom: 1px solid #ccc;"></td>
				</tr>
				<tr>
				...
				<tr id="row-jml-partisipasi-80" style="display:none;">
					<th style="text-align: left; padding: 5px; border-bottom: 1px solid #ccc;">Partisipasi 80</th>
					<td id="jml-partisipasi-80" style="padding: 5px; border-bottom: 1px solid #ccc;"></td>
				</tr>
				<tr>
					<th style="text-align: left; padding: 5px; border-bottom: 1px solid #ccc;">Relawan</th>
					<td id="jml-relawan" style="padding: 5px; border-bottom: 1px solid #ccc;"></td>
				</tr>
				<tr id="row-jml-target-kk-terkunjungi" style="display:none;">
					<th style="text-align: left; padding: 5px; border-bottom: 1px solid #ccc;">Target KK Terkunjungi</th>
					<td id="jml-target-kk-terkunjungi" style="padding: 5px; border-bottom: 1px solid #ccc;"></td>
				</tr>
				<tr id="row-jml-pemilih-terkunjungi" style="display:none;">
					<th style="text-align: left; padding: 5px; border-bottom: 1px solid #ccc;">Pemilih Terkunjungi</th>
					<td id="jml-pemilih-terkunjungi" style="padding: 5px; border-bottom: 1px solid #ccc;"></td>
				</tr>
				<tr>
					<th style="text-align: left; padding: 5px; border-bottom: 1px solid #ccc;">Target Suara</th>
					<td id="jml-target-suara" style="padding: 5px; border-bottom: 1px solid #ccc;"></td>
				</tr>
				<tr id="row-jml-percentage-target-suara" style="display:none;">
					<th style="text-align: left; padding: 5px; border-bottom: 1px solid #ccc;">% Target Suara</th>
					<td id="jml-percentage-target-suara" style="padding: 5px; border-bottom: 1px solid #ccc;"></td>
				</tr>
				<tr>
					<th style="text-align: left; padding: 5px; border-bottom: 1px solid #ccc;">Jumlah Spanduk</th>
					<td id="jml-spanduk" style="padding: 5px; border-bottom: 1px solid #ccc;"></td>
				</tr>
				<tr>
					<th style="text-align: left; padding: 5px; border-bottom: 1px solid #ccc;">Jumlah Baliho</th>
					<td id="jml-baliho" style="padding: 5px; border-bottom: 1px solid #ccc;"></td>
				</tr>
			</tbody>
		</table>
	</div>`;
	...
	function showDataTooltip(data) {
		...
		if (parseInt(data.region_level) === CONST_CITY_LEVEL) {
			...
			document.getElementById("row-jml-partisipasi-80").style.display = "none";
			document.getElementById("row-jml-target-kk-terkunjungi").style.display = "none";
			document.getElementById("row-jml-pemilih-terkunjungi").style.display = "none";
			document.getElementById("row-jml-percentage-target-suara").style.display = "none";
			...
			document.getElementById("row-jml-partisipasi-80").style.display = "table-row";
			document.getElementById("jml-partisipasi-80").textContent = data.jml_partisipasi_80;
			document.getElementById("row-jml-target-kk-terkunjungi").style.display = "table-row";
			document.getElementById("jml-target-kk-terkunjungi").textContent =
				data.jml_target_kk_terkunjungi;

			document.getElementById("row-jml-pemilih-terkunjungi").style.display = "table-row";
			document.getElementById("jml-pemilih-terkunjungi").textContent =
				data.jml_pemilih_terkunjungi;

			document.getElementById("row-jml-percentage-target-suara").style.display = "table-row";
			document.getElementById("jml-percentage-target-suara").textContent =
				(Number(data.jml_percentage_target_suara) * 100).toFixed(2) + "%";
		}

		document.getElementById("jml-dpthp").textContent = data.jml_dpthp2;
		document.getElementById("jml-dpthp-perempuan").textContent = data.jml_dpthp2_perempuan;
		document.getElementById("jml-relawan").textContent = data.jml_relawan;
		document.getElementById("jml-target-suara").textContent = data.jml_target_suara;
		document.getElementById("jml-spanduk").textContent = data.jml_total_spanduk;
		document.getElementById("jml-baliho").textContent = data.jml_total_baliho;
	}
		...
		data.forEach((item) => {
			...
			if (parseInt(level) == CONST_CITY_LEVEL) {
				voterData += `
					...
					<td>${item.target_voter}</td>
				`;
			} else if (parseInt(level) == CONST_DISTRICT_LEVEL) {
				voterData += `
					<td>${item.volunteer}</td>
					<td>${item.target_visited_kk}</td>
					<td>${item.visited_voter}</td>
					<td>${item.target_voter}</td>
					<td>${item.percentage_target_voter * 100}%</td>
				`;
			}
            ...
            <td>${item.zone}</td>
			<td>${item.num_voter_dpthp2}</td>
			<td>${item.num_voter_women_dpthp2}</td>
			${voterData}
			<td>${item.total_baliho}</td>
			<td>${item.total_spanduk}</td>
            </tr>
          `;
		});
	...
	function fetchTableData(level, region) {
		const url = `polmark_dashboard.api.tabular.get_tabular_data?region=Kota Bandung&region_level=${mapRenderLevel}&region_code=${region}`;
