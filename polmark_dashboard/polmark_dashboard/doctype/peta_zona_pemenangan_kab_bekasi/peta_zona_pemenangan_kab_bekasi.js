...
function initializeLeafletMap(frmInstance) {
	...
	currentMapLevel = parseInt(region_level);
	...
	addBackButton(mapInstance, frmInstance, region, region_level);
	addLegend(mapInstance);
	addMapTitleLabel(mapInstance, `${region_type} ${region_name}`);
	addShowHideLayer(mapInstance);

	console.log('getMarkersGroup(region_level): ', getMarkersGroup(region_level));

	// Initial load of map
	fetchGeoJsonDataByRegion({
		frm: frmInstance,
		map: mapInstance,
		markersGroup: getMarkersGroup(region_level),
		level: parseInt(region_level),
		regionCode: region,
		regionName: region_name,
		regionType: region_type,
		zoomLevel: 5,
		defaultView: getDefaultMapView(region_level),
	});
}
